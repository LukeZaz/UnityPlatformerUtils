...
public class TileColliderManager : MonoBehaviour {
        ...
        void Start ()
        {
                if (colliderMaterial == null)
                        Debug.LogWarning ("Collider Material not specified for TileColliderManager. None will be applied.");
                ...
                while (tiles.Count > 0)
                {
                        ...
                        while (cont && tiles.Count > 0)
                        {
                                ...
                                for (int i = 0; i < tiles.Count; i++)
                                {
                                        ...
                                        if (inRange (pos.x, check.x - 0.01f, check.x + 0.01f)
                                            && inRange (pos.y, parentPos.y - 0.01f, parentPos.y + 0.01f))
                                        {
                                                // Left
                                        ...
                                        else if (inRange (pos.x, check.y - 0.01f, check.y + 0.01f)
                                                 && inRange (pos.y, parentPos.y - 0.01f, parentPos.y + 0.01f))
                                        {
                                                // Right
                                                cont = true;
                ...
                while (cols.Count > 0)
                {
                        ...
                        while (cont && cols.Count > 0)
                        {
                                ...
                                for (int i = 0; i < cols.Count; i++)
                                {
                                        ...
                                        if (inRange (pos.y, checkUp-0.01f, checkUp+0.01f)
                                        ...
                                        else if (inRange (pos.y, checkDown-0.01f, checkDown+0.01f)
                                                 && inRange (pos.x, selfPos.x-0.01f, selfPos.x+0.01f)
                                                 && cols[i].size.x == col.size.x)
                                        {
                                                // Down
