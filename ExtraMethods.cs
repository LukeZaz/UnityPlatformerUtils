...
public class ExtraMethods : MonoBehaviour {
	...
	/* 
	 * ExtraMethods is a script that itself does nothing, but is around to provide some extra useful
	...
	public bool inRange (int num, int first, int second)
	{
		if (first > second)
		{
			if (num >= second && num <= first)
	...
	public bool inRange (double num, double first, double second)
	{
		if (first > second)
		{
			if (num >= second && num <= first)
